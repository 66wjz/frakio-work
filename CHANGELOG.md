...
## 1.4.0 — 一个人，开始拥有一支真正能做事的 AI 团队

Frakio Work 1.4.0 把多 Agent 协作从“大家在同一个对话里交流”，带到了“大家可以围绕同一个目标持续完成工作”。

现在，你可以先让 Agent 帮你整理方案。普通计划适合先调查、分析和规划，由一个 Agent 把事情想清楚；协作计划则适合需要多人分工的任务，确认后会形成一条清晰的协作流程。研究、撰写、审核、交付，分别由合适的 Agent 接手，你可以随时看到事情进行到哪一步。

协作开始后，不需要一直盯着页面。每个任务都有自己的进度、依赖和结果；某一步需要等待前面的工作时，系统会明确告诉你原因。所有工作完成后，协调 Agent 会把结果整理成一次正式交付，而不是让你自己从零散对话里拼答案。

执行过程中有了新想法，也不必重新开一轮任务。直接在当前对话里补充、调整或 @ 某位 Agent，内容会作为对正在进行的协作的引导，让团队顺着新的方向继续做下去。

这次也加入了收件箱。任务完成、需要你确认、等待你的回答，都会集中在这里。你不必反复翻找对话，重要事情会主动找到你。

Plan 模式也更接近真实的“先想清楚，再开始”。Agent 现在可以在不修改项目的前提下阅读文件、搜索资料、浏览网页、进行必要的调查操作；但在你确认之前，它不会改代码、发布内容、删除文件或动用任何需要授权的能力。

Frakio Work 也继续完善了多内核体验。无论 Agent 使用 Hermes、Pi、Codex 还是 Claude Code，模型、上下文、运行状态和恢复过程都会由 Frakio 统一管理。切换对话、刷新页面或重启应用后，正在进行的工作更容易回到正确的位置。

这不是一次单纯的功能增加。1.4.0 是 Frakio Work 从多 Agent 工作台继续走向 AI 协作组织的一步。你负责目标和判断，Agent 团队负责把事情持续推进到交付。

## 1.3.1 — 2026-08-07

Runtime Center 修复 Codex 与 Claude Code 的托管安装。托管包在正式目录重新验证稳定指纹，旧版路径指纹会明确提示重新安装；同版本重装会安全替换失效 Build，启用失败不会切换当前 Runtime。安装过程现在会自动展开目标 Runtime，显示下载与验证中的明确反馈，完成后才允许启用。

会话运行展示与 Agent 接力继续收敛：事件去重、持久化交接、运行状态恢复和 Harness 选择在重连与刷新后保持一致。设置页同步完善 Agent Profile 编辑、Runtime 状态显示和本地 API 请求校验，避免状态错误地显示为可用或吞掉失败原因。
...
Pi、Codex、Claude Code 与 Hermes 的执行过程统一为同一条时间线。模型正文与工具调用会按真实发生顺序交错显示；Pi 不再把全部工具活动堆在最终回复前。工具结束后等待结果时，界面会显示“正在整理结果”，不会展示模型的隐藏推理内容。

多会话并行与 Agent 接力的终态处理进一步收敛。重复事件、恢复后的原生终态和已删除会话会被幂等处理，避免重复回复、遗留运行或输入区错误锁定。
...
上下文协调器会按目标模型的上下文窗口计算软阈值和硬阈值，在后台生成可移植检查点。压缩只影响发送给模型的上下文，不删除原始会话账本；压缩开始、完成和失败都能在界面中观察，失败时会明确保留原始历史。

Run 控制改为由 Host Run ID 前置管理。启动、工具执行、压缩和审批阶段都支持统一停止，启动阶段也可以立即取消；每个 Run 只会产生一个终态。应用重启、Runtime 切换或原生 Session 恢复失败时，会使用检查点和事件尾部重建上下文，避免任务和对话丢失。

命令、文件、网络、发布、删除、授权和记忆写入统一经过 Permission Broker。会话设置优先于工作区和 Agent 默认值，Runtime 无法观察的能力会如实标记，避免界面显示实际上不能生效的控制项。

多会话并行运行继续保留。草稿按工作区和会话隔离并持久化，多窗口通过事件游标重放运行状态，切换项目、设置或窗口后不会丢失输入、工具状态、压缩记录或最终回复。

## 1.1.1 — 2026-07-31

删除 Agent 时现在会先停止对应的 Profile Gateway，再同步清理 Agent 注册表、Gateway 自动启动清单、选中 Profile、创建请求记录和 Profile 目录；并发删除和历史遗留 Profile 也会保持幂等，避免 Runtime 重启后重新生成孤儿目录。

模型、推理强度和速度选择器改为稳定的点击进入面板，避免 React 19 与 Radix 嵌套子菜单在碰撞翻转时吞掉点击。保存操作会等待对应的会话 PATCH 请求并在失败时保留原状态。个人资料编辑弹窗改为固定头部、独立滚动内容和固定底部操作区，背景滚动会被锁定。

托管 Web 首次安装使用默认管理员密码 `Admin`，首次登录后强制设置至少 10 位的新密码；修改后的密码会失效默认密码，桌面版继续使用本机安全会话自动登录。
