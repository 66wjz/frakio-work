...
| **阶段 5** | 运行时 500 排查与模型通道修复 | 修复路由重复拦截，补齐 Kimi 等模型桥接解析 | 端到端真机测试成功回复，对话链路畅通 | **已完成** |
| **阶段 6** | Base/Composite 组件库与页面全面替换 | 建立 Base/Composite/BaseIcon 体系与统一高度 Token，全面替换业务页面硬编码 | 控件高度/层级 Token 对齐，业务页面全面应用新组件，类型检查 0 error | **进行中** |
...
- [x] **5.1 修复工作区与会话路由冲突，增强 Python Bridge 对自定义 Provider 解析**
- [x] **5.2 建立 Fork + Upstream 双远程源版本管理并推送个人 GitHub**
- [x] **6.1 封装 Base (15个) + Composite (6个) 组件库与 BaseIcon 统一图标中心**（`components/base/`, `components/composite/`）
- [x] **6.2 编写前端设计规范手册与组件指南**（`docs/前端设计规范与Token体系手册.md`, `BASE_COMPONENTS_GUIDE.md`）
- [/] **6.3 系统性替换设置中心、工作台与会话界面的散落组件与图标**
