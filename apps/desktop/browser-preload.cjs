...
function ensureOverlay() {
  ...
  host.setAttribute('data-frakio-annotation-layer', '');
...
function saveAnnotation() {
  ...
  ipcRenderer.send('frakio:browser-annotation', annotation);
  state.markerCount += 1;
...
ipcRenderer.on('frakio-browser:set-mode', (_event, mode) => updateMode(mode));
